...
const d1 = findDistances(findGalaxies(grid));
console.log("d1\t", d1);

const inflatedGrid = inflateGrid(grid);
const d2 = findDistances(findGalaxies(inflatedGrid));
console.log("d2\t", d2);

// "diff" is the number of extra hops between the original grid
// and the grid that was inflated by a factor of 1. (i.e., each
// empty row or column was doubled).
const diff = d2 - d1;
...
const newSum = mx * diff + d1 - diff;

console.log();
console.group(newSum);
